...
- [Устройство Linux](#устройство-linux)
  ...
  - [🛠️ Команды для перформанса и глубокой отладки](#️-команды-для-перформанса-и-глубокой-отладки)
  - [| `kubectl debug pod/app -it --copy-to=debug-pod --image=nicolaka/netshoot` | Глубокая сетевая отладка с использованием netshoot toolbox. |](#-kubectl-debug-podapp--it---copy-todebug-pod---imagenicolakanetshoot--глубокая-сетевая-отладка-с-использованием-netshoot-toolbox-)
  - [🗂️ Структура каталогов Linux (FHS)](#️-структура-каталогов-linux-fhs)
...
> Более подробное описание старта можно прочитать вот [тут](linux-boot-deep-dive.md)
---
...
## 🛠️ Команды для перформанса и глубокой отладки

> Когда дашбордов уже недостаточно — переходишь на уровень ОС.  
> Эти команды используются во время продакшен-инцидентов.

| Команда | Что делает |
|---------|------------|
| `strace -p <PID> -c` | Трейсит системные вызовы процесса с агрегированной статистикой. Помогает понять, чего он ждёт. |
| `perf top -p <PID>` | Живой CPU-профайлинг процесса. Мгновенно показывает «горячие» функции. |
| `tcpdump -i eth0 -w capture.pcap port 8080` | Сниффинг трафика на порту 8080. Глубокая отладка сети. |
| `ab -n 10000 -c 100 http://localhost/` | Нагрузочное тестирование через Apache Bench. 10k запросов, 100 одновременных. |
| `kubectl debug node/node1 -it --image=busybox` | Подключение ephemeral-контейнера для отладки ноды Kubernetes. |
| `dmesg -T \| grep -i 'oom'` | Проверка OOM-событий (killer) в ядре с таймстемпами. |
| `lsof -i :8080` | Показывает, какой процесс занимает порт 8080. |
| `iostat -xz 1 5` | Статистика дискового I/O каждую секунду (5 итераций). |
| `vmstat -w 1 5` | Метрики памяти, swap и CPU в реальном времени. |
| `kubectl debug pod/app -it --copy-to=debug-pod --image=nicolaka/netshoot` | Глубокая сетевая отладка с использованием netshoot toolbox. |
---

## 🗂️ Структура каталогов Linux (FHS)

> **FHS (Filesystem Hierarchy Standard)** — стандарт, который определяет, где должны лежать файлы в Linux.  
> Благодаря нему программы и пакетные менеджеры знают, куда класть бинарники, конфиги и библиотеки.

| Каталог | Что там лежит |
|---------|---------------|
| **`/bin`** | Базовые команды, без которых система не сможет нормально работать |
| **`/boot`** | Ядро (`vmlinuz`), `initramfs` и файлы загрузчика |
| **`/dev`** | Файлы устройств (жёсткие диски, терминалы, порты) |
| **`/etc`** | Общесистемные конфиги |
| **`/home`** | Домашние каталоги пользователей |
| **`/lib`** | Общие библиотеки (`.so`) и модули ядра |
...
| **`/var`** | Логи, кэш и прочие данные, которые часто меняются |

> **Важно:** в современных дистрибутивах `/bin`, `/sbin`, `/lib` часто являются **символическими ссылками** на `/usr/bin`, `/usr/sbin`, `/usr/lib`.
